...
class BBQV5NoLSNoInit(torch.nn.Module):
    def __init__(
        self,
        precision: int, 
        zero_point: float,
        ...
        self.sx = torch.nn.Parameter(torch.tensor(1.0), requires_grad=False)
...
class BBQV5NoLSNoInitHD(BBQV5NoLSNoInit):
    ...
    def extra_repr(self):
        return f"precision={self.precision}, n={self.n_levels}, hn={self.half_n_levels}, per_channel_rms={self.per_channel_rms}, zero_point={self.zero_point}, ema_rrms={self.ema_rrms}"
    
class BBQV5NoLSNoInitHDChan(BBQV5NoLSNoInitHD):
    def __init__(
        self,
        precision: int, 
        zero_point: float,
