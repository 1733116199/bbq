...
from .quantization import QuantizedLinear, QUANTIZER_CLASSES, BBQV5HD, BBQV5NoHD, BBQV5NoLSHD, BBQV5NoLSNoInitHD, BBQV5NoRMSHD, LTQ, LTQW
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        w_quant_kwargs = {**config.w_quant_kwargs}
        if "channels" in w_quant_kwargs:
            w_quant_kwargs["channels"] = 3 * config.n_embd
        self.c_attn = QuantizedLinear(
            config.n_embd,
            3 * config.n_embd,
            bias=config.bias,
            weight_quantizer=QUANTIZER_CLASSES[config.w_quant](**w_quant_kwargs),
        ...
        w_quant_kwargs = {**config.w_quant_kwargs} 
        if "channels" in w_quant_kwargs:
            w_quant_kwargs["channels"] = config.n_embd
...
class MLP(nn.Module):
    def __init__(self, config, exp_factor=1.0):
        ...
        w_quant_kwargs = {**config.w_quant_kwargs}
        if "channels" in w_quant_kwargs:
            w_quant_kwargs["channels"] = int(self.dim_exp_factor * config.n_embd)
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith("bias"):
                    ...
                    no_decay.add(fpn)
                elif pn.endswith("sx") and isinstance(m, (BBQV5HD, BBQV5NoHD, BBQV5NoLSHD, BBQV5NoLSNoInitHD, BBQV5NoRMSHD)):
                    # my own scaling factors will NOT be weight decayed
                    print(fpn, "no weight decay!!!")
                    no_decay.add(fpn)
                elif (pn.endswith("clip_val") or pn.endswith("zero")) and isinstance(m, (LTQW,)):
                    print(fpn, "no weight decay!!!")
                    no_decay.add(fpn)
                elif (pn.endswith("start") or pn.endswith("a") or pn.endswith("scale1") or \
                      pn.endswith("scale2") or pn.endswith("two") or pn.endswith("one") or \
                        pn.endswith("zero") or pn.endswith("minusone") or pn.endswith("eps") or \
                        pn.endswith("interval") ) and isinstance(m, (LTQ,)):
                    print(fpn, "no weight decay!!!")
                    no_decay.add(fpn)
                elif pn.endswith("weight_clip_val"):
                    # ESQ scaling factors will NOT be weight decayed
                    print(fpn, "no weight decay!!!")
                    no_decay.add(fpn)
                elif pn.endswith("boundaries") or pn.endswith("data_type"):
                    # ESQ scaling factors will NOT be weight decayed
                    print(fpn, "no weight decay!!!")
                    no_decay.add(fpn)
