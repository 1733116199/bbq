...
from models.quantization.base_linear import BBQV5HD
...
def eval(
    ...
    keys = ["gemm", "bbq", "elementwise", "reduce", "softmax", "gather", "memset", "memcpy"]
    total_time_records = []
    categorized_records = {**{k: [] for k in keys}, "nf4": [], "other": []}
    i = 0
    for idx in trange(max_num_batches):
        ...
        if i >= cfg.profile_start and i < cfg.profile_end:
            ...
            total_time = 0
            categorized = {**{k: 0 for k in keys}, "nf4": 0, "other": 0}
            for e in prof.events():
                if hasattr(e, "cuda_time"):
                    ...
                    if e.cuda_time == 0.0:
                        continue
                    total_time += e.device_time
                    found = False
                    for k in keys:
                        if k in e.name.lower():
                            found = True
                            categorized[k] += e.device_time
                            break
                    if not found:
                        if "flash" in e.name.lower():
                            categorized["gemm"] += e.device_time
                        if "attention" in e.name.lower():
                            categorized["gemm"] += e.device_time
                        elif "autocast" in e.name.lower():
                            categorized["elementwise"] += e.device_time
                        elif "quantizeblockwise" in e.name.lower():
                            categorized["nf4"] += e.device_time
                        else:
                            print(e.name)
                            print("\n\n")
                            categorized["other"] += e.device_time

            total_time_records.append(total_time)
            for k in categorized_records:
                categorized_records[k].append(categorized[k])
            
        elif i >= cfg.profile_end:
            wandb.log({
                "profile/mean/total": np.array(total_time_records).mean(), 
                **{f"profile/mean/{k}": np.array(categorized_records[k]).mean() for k in categorized_records},
                "profile/median/total": np.median(np.array(total_time_records)), 
                **{f"profile/median/{k}": np.median(np.array(categorized_records[k])) for k in categorized_records}
            })
            import sys
            sys.exit(0)
        else:
