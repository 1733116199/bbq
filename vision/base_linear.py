...
class NoQuantizer(BaseQuantizer):
    def __init__(self, **kwargs):
        ...
        self.rounding_error = torch.tensor(0.0)
...
class HalfHadamardTrustQuantizer(STEQuantizer):
    ...
    def __init__(self, bits=4, trust=None, lhd=5, **kwargs):
    ...
    def forward(self, x: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                self.rounding_error = torch.abs(temp - (x_clip / step + 1 / 2)).mean()
            self.log_ent(temp)
            xq = temp * step - step / 2
...
class LSQQuantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.raise_zero:
            ...
            with torch.no_grad():
                self.rounding_error = torch.abs(xscr - xsc).mean()
            self.log_ent(xscr)
            xscr = xscr + 1 / 2
        else:
...
class BBQNaiveBase(torch.nn.Module):
    def __init__(
        self,
        precision: int, 
        zero_point: float,
    ...
    def quant(self, x: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            self.mul_step = (sx.detach() / self.half_n_levels)
...
class BBQNaiveHD(BBQNaiveBase):
    aux_matrix = hadamard_transform(
        torch.eye(32, dtype=torch.float32, device="cuda"), scale=2 ** (-5 / 2)
    )
    def __init__(
        self,
        precision: int, 
...
class BBQNaiveHDChan(BBQNaiveHD):
    def __init__(
        self,
        precision: int, 
        zero_point: float,
...
class BBQVision(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_forward = torch.round(x).clip(0, self.n_levels - 1) # within range [0, 2^p-1]
        with torch.no_grad():
            self.rounding_error = torch.abs(x_forward - x).mean()
...
QUANTIZER_CLASSES = {
    ...
    "BBQNaiveHD": BBQNaiveHD,
    "BBQNaiveHDChan": BBQNaiveHDChan,
