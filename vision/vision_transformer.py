...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., config=None):
        ...
        self.fc1 = QuantizedLinear(
            ...
            weight_quantizer=QUANTIZER_CLASSES[config.w_quant](**config.w_quant_args, channels=hidden_features),
            activation_quantizer=QUANTIZER_CLASSES[config.a_quant](**config.a_quant_args),
        )
        self.act = act_layer()
        self.fc2 = QuantizedLinear(
            hidden_features, out_features,
            weight_quantizer=QUANTIZER_CLASSES[config.w_quant](**config.w_quant_args, channels=out_features),
            activation_quantizer=QUANTIZER_CLASSES[config.a_quant](**config.a_quant_args, all_positive=True, raise_zero=False),
        )
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., config=None):
        ...
        self.qkv = QuantizedLinear(dim, dim * 3, bias=qkv_bias,
            weight_quantizer=QUANTIZER_CLASSES[config.w_quant](**config.w_quant_args, channels=dim * 3),
            activation_quantizer=QUANTIZER_CLASSES[config.a_quant](**config.a_quant_args),
        )

        self.query_quantizer = QUANTIZER_CLASSES[config.qksv_quant](**config.qksv_quant_args, head=self.num_heads)
        self.key_quantizer = QUANTIZER_CLASSES[config.qksv_quant](**config.qksv_quant_args, head=self.num_heads)
        self.score_quantizer = QUANTIZER_CLASSES[config.qksv_quant](**config.qksv_quant_args, head=self.num_heads)
        self.value_quantizer = QUANTIZER_CLASSES[config.qksv_quant](**config.qksv_quant_args, head=self.num_heads)
        self.hd_block_size = (1 << config.qksv_quant_args["lhd"]) if "lhd" in config.qksv_quant_args else 64
        print("self.hd_block_size=", self.hd_block_size)
        
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = QuantizedLinear(dim, dim,
            weight_quantizer=QUANTIZER_CLASSES[config.w_quant](**config.w_quant_args, channels=dim),
            activation_quantizer=QUANTIZER_CLASSES[config.a_quant](**config.a_quant_args),
        )
        self.proj_drop = nn.Dropout(proj_drop)

    def pad_middle(self, x):
        n_splits = math.ceil(x.shape[-1] / self.hd_block_size)
        split_size = math.ceil(x.shape[-1] / n_splits)
        splits = torch.split(x, split_size, dim=-1)
        return torch.cat([torch.nn.functional.pad(s, (0, self.hd_block_size - s.shape[-1])) for s in splits], dim=-1)
    ...
    def forward(self, x):
        ...
        q = self.query_quantizer(q)
        k = self.key_quantizer(k)
        v = self.value_quantizer(
            self.pad_middle(v.transpose(-2, -1))
        ).transpose(-1, -2)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        attn = self.score_quantizer(self.pad_middle(attn))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, config=config)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', config=None):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, config=config)
    ...
    def no_weight_decay(self):
        res = {'pos_embed', 'cls_token', 'dist_token'}
        for name, parameter in self.named_parameters():
            if name.endswith(".sx") or name.endswith(".sx1") or name.endswith(".sx2") or name.endswith(".zeta"):
                res.add(name)
        print("no_weight_decay", res)
        return res
