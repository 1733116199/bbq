...
export W_QUANT_KWARGS="{\"bits\": $PRECISION}"
export A_QUANT_KWARGS="{\"bits\": $PRECISION}"
...
WANDB_PREFIX="NEW-wiki-${MODEL_SIZE_PREFIX}-${W_QUANT}@${PRECISION}:${A_QUANT}@${PRECISION}-${DATASET}"
...
torchrun --nproc_per_node=${NUM_GPUS} ./src/main.py \
    ...
    --dataset "wikitext" \
    --resume-from exps/NEW-200M-HalfHadamardTrustQuantizer@${PRECISION}:HalfHadamardTrustQuantizer@${PRECISION}-c4_c4_llama_nlayers10_nhead10_lr0.0003_sched_cos_warmup7629_decay_linear_0.1_iter76293_bs128x4_ws1_seed0_data_seed1337/ckpts/latest \
    --w-quant ${W_QUANT} \
